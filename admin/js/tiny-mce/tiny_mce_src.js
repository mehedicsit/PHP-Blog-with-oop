	...
	tinymce.php.SaxParser = function(settings, schema) {
		...
		self.parse = function(html) {
			var self = this, matches, index = 0, value, endRegExp, stack = [], attrList, i, text, name, isInternalElement, removeInternalElements,
				...
				tokenRegExp, attrRegExp, specialElements, attrValue, idCount = 0, decode = tinymce.php.Entities.decode, fixSelfClosing, isIE;
							...
							if (parent.isEmpty(nonEmptyElements)) {
								...
								if (elementRule) {
								  if (elementRule.removeEmpty)
									  ...
									  parent.empty().append(new tinymce.php.Node('#text', 3)).value = '\u00a0';
							  }
              }
						}
					}
...
tinymce.php.Writer = function(settings) {
	...
	encode = tinymce.php.Entities.getEncodeFunc(settings.entity_encoding || 'raw', settings.entities);
...
(function(tinymce) {
	tinymce.php.Serializer = function(settings, schema) {
		var self = this, writer = new tinymce.php.Writer(settings);
		...
		setupIframe : function() {
			...
			t.parser.addNodeFilter('p,h1,h2,h3,h4,h5,h6,div', function(nodes, name) {
				...
				while (i--) {
					...
					if (node.isEmpty(nonEmptyElements))
						node.empty().append(new tinymce.php.Node('br', 1)).shortEnded = true;
				}
			});
				...
				if (failed || !doc.queryCommandSupported(command)) {
					if (tinymce.isGecko) {
						editor.windowManager.confirm(editor.getLang('clipboard_msg'), function(state) {
							if (state)
								open('http://www.mozilla.org/editor/midasdemo/securityprefs.php', '_blank');
						});
					} else
